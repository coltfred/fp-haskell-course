...
fiilter ::
  ...
  -> List a
fiilter _ Nil = Nil
fiilter f (hd :. rest) = if(f(hd)) then hd :. fiilter f rest else fiilter f rest 
...
seqOptional = foldLeft (\acc x -> acc `bindOptional` (\accList ->  x `bindOptional` (\item -> Full (item :. accList)) )) (Full Nil)
